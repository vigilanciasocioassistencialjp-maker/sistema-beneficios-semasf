...
def login():
    ...
    if len(tentativas_login[ip]) >= MAX_TENTATIVAS:
        ...
        return render_template("login.html", erro=f"⛔ Muitas tentativas! Aguarde {minutos_restantes} minutos.", bloqueado=True)
    if request.method == "POST":
        ...
        conexao.close()
        if dados:
            usuario_banco, senha_hash, perfil_banco, primeiro_acesso, cras_banco, nome_usuario = dados[0], dados[1], dados[2], dados[3] if len(dados) > 3 else 0, dados[4] if len(dados) > 4 else None, dados[5] if len(dados) > 5 and dados[5] else dados[0]
            try:
                if bcrypt.checkpw(senha.encode('utf-8'), senha_hash.encode('utf-8')):
                    ...
                    erro = f"❌ Senha incorreta! Tentativas restantes: {tentativas_restantes}" if tentativas_restantes > 0 else "⛔ Conta bloqueada por 15 minutos."
            except Exception as e:
            ...
            erro = f"❌ Usuário não encontrado! Tentativas restantes: {tentativas_restantes}" if tentativas_restantes > 0 else "⛔ Conta bloqueada por 15 minutos."
    ...
    if request.method == "POST":
        ...
        if not nova_senha or not confirmar_senha:
            ...
            if not erro:
                ...
                conexao = get_db()
                cursor = conexao.cursor()
                cursor.execute("UPDATE usuarios SET senha = %s, primeiro_acesso = 0 WHERE usuario = %s", (nova_senha_hash, current_user.id))
                conexao.commit()
                cursor.close()
                conexao.close()
            ...
            if primeiro_acesso:
                if current_user.perfil in ['admin', 'gestor']:
                    ...
                    return redirect(url_for("solicitacoes"))
    return render_template("trocar_senha.html", erro=erro, sucesso=sucesso, primeiro_acesso=primeiro_acesso)
...
def verificar_cpf(cpf):
    """Verifica validade e histórico de cestas do CPF usando hash"""
    try:
        ...
        # Contar total de cestas entregues usando hash
        cursor.execute("""
        ...
        cursor.execute("""
            ...
            WHERE cpf_hash = %s AND status = 'Cadastrada'
        """, (hash_busca,))
        ...
        cursor.execute("""
            ...
            WHERE cpf_hash = %s ORDER BY id DESC LIMIT 1
        """, (hash_busca,))
...
# =====================================================
# PÁGINA PRINCIPAL (Nova Solicitação) - COM HASH
# =====================================================
...
def inicio():
    if request.method == "POST":
        ...
        """, (
            tecnico, cpf_criptografado, cpf_hash, nome, data_nascimento, telefone, email,
            endereco, numero, complemento, bairro, cep, referencia,
...
def solicitacoes(pagina=1):
    ...
    if current_user.perfil in ['admin', 'gestor']:
        ...
        cursor.execute("""
            ...
            FROM solicitacoes ORDER BY id DESC LIMIT %s OFFSET %s
        ...
        cursor.execute("""
            SELECT id, tecnico, nome, cpf, bairro, cras, data_solicitacao, status
            FROM solicitacoes WHERE cras = %s ORDER BY id DESC LIMIT %s OFFSET %s
...
def registrar_entrega(id):
    ...
    if current_user.perfil not in ['admin', 'gestor']:
        ...
        if not result or result[0] != current_user.cras:
            ...
            return redirect(url_for("solicitacoes"))
    cursor.execute("UPDATE solicitacoes SET status = %s, data_entrega = %s, tecnico_entrega = %s WHERE id = %s", (status_entrega, data_entrega, current_user.id, id))
    if observacoes:
        cursor.execute("UPDATE solicitacoes SET parecer = parecer || '\n\n--- OBSERVAÇÕES DA ENTREGA ---\n' || %s WHERE id = %s", (observacoes, id))
    conexao.commit()
    conexao.close()
    logger.info(f"Entrega registrada: ID={id}, Status={status_entrega}, Técnico={current_user.id}")
    flash('✅ Entrega registrada com sucesso!' if status_entrega == 'Entregue' else '❌ Ausência registrada.', 'success' if status_entrega == 'Entregue' else 'warning')
    return redirect(url_for("solicitacoes"))
...
def dashboard():
    ...
    conexao.close()
    return render_template("dashboard.html", total_solicitacoes=total_solicitacoes, total_entregues=total_entregues, total_ausentes=total_ausentes, total_pendentes=total_pendentes, datetime=datetime, current_user=current_user)
...
def relatorio():
    ...
    conexao = get_db()
    cursor = conexao.cursor()
    cursor.execute("SELECT COUNT(*) FROM solicitacoes WHERE data_solicitacao IS NOT NULL AND SUBSTR(data_solicitacao, 7, 4) = %s AND SUBSTR(data_solicitacao, 4, 2) = %s", (ano, mes_num))
    total_solicitacoes = cursor.fetchone()[0] or 0
    cursor.execute("SELECT COUNT(*) FROM solicitacoes WHERE status = 'Entregue' AND data_entrega IS NOT NULL AND ((SUBSTR(data_entrega, 7, 4) = %s AND SUBSTR(data_entrega, 4, 2) = %s) OR (SUBSTR(data_entrega, 1, 4) = %s AND SUBSTR(data_entrega, 6, 2) = %s))", (ano, mes_num, ano, mes_num))
    total_entregues = cursor.fetchone()[0] or 0
    cursor.execute("SELECT COUNT(*) FROM solicitacoes WHERE status = 'Ausente' AND data_entrega IS NOT NULL AND ((SUBSTR(data_entrega, 7, 4) = %s AND SUBSTR(data_entrega, 4, 2) = %s) OR (SUBSTR(data_entrega, 1, 4) = %s AND SUBSTR(data_entrega, 6, 2) = %s))", (ano, mes_num, ano, mes_num))
    total_ausentes = cursor.fetchone()[0] or 0
    cursor.execute("SELECT COUNT(*) FROM solicitacoes WHERE status = 'Cadastrada' AND data_solicitacao IS NOT NULL AND SUBSTR(data_solicitacao, 7, 4) = %s AND SUBSTR(data_solicitacao, 4, 2) = %s", (ano, mes_num))
    total_pendentes = cursor.fetchone()[0] or 0
    cursor.execute("SELECT cras, COUNT(*) as total, SUM(CASE WHEN status = 'Entregue' THEN 1 ELSE 0 END) as entregues, SUM(CASE WHEN status = 'Ausente' THEN 1 ELSE 0 END) as ausentes FROM solicitacoes WHERE data_solicitacao IS NOT NULL AND SUBSTR(data_solicitacao, 7, 4) = %s AND SUBSTR(data_solicitacao, 4, 2) = %s GROUP BY cras", (ano, mes_num))
    por_cras = cursor.fetchall()
    cursor.execute("SELECT COALESCE(u.nome, 'Técnico não identificado') as nome_tecnico, COUNT(*) as total, SUM(CASE WHEN s.status = 'Entregue' THEN 1 ELSE 0 END) as entregues, SUM(CASE WHEN s.status = 'Ausente' THEN 1 ELSE 0 END) as ausentes FROM solicitacoes s LEFT JOIN usuarios u ON s.tecnico_entrega = u.usuario WHERE s.status IN ('Entregue', 'Ausente') AND s.data_entrega IS NOT NULL AND ((SUBSTR(s.data_entrega, 7, 4) = %s AND SUBSTR(s.data_entrega, 4, 2) = %s) OR (SUBSTR(s.data_entrega, 1, 4) = %s AND SUBSTR(s.data_entrega, 6, 2) = %s)) GROUP BY s.tecnico_entrega, u.nome", (ano, mes_num, ano, mes_num))
    por_tecnico = cursor.fetchall()
    cursor.execute("SELECT s.nome, s.cpf, s.bairro, s.cras, s.status, s.data_entrega, COALESCE(u.nome, 'Não informado') as tecnico_nome FROM solicitacoes s LEFT JOIN usuarios u ON s.tecnico_entrega = u.usuario WHERE s.status IN ('Entregue', 'Ausente') AND s.data_entrega IS NOT NULL AND ((SUBSTR(s.data_entrega, 7, 4) = %s AND SUBSTR(s.data_entrega, 4, 2) = %s) OR (SUBSTR(s.data_entrega, 1, 4) = %s AND SUBSTR(s.data_entrega, 6, 2) = %s)) ORDER BY s.data_entrega DESC LIMIT 20", (ano, mes_num, ano, mes_num))
    ultimas_entregas = cursor.fetchall()
    # Recorrência
    cursor.execute("SELECT cpf, COUNT(*) as total_recebido, MAX(data_entrega) as ultima_entrega FROM solicitacoes WHERE status = 'Entregue' AND cpf IS NOT NULL GROUP BY cpf HAVING COUNT(*) > 1 ORDER BY total_recebido DESC LIMIT 20")
    ...
    for r in recorrencia_raw:
        ...
        total = r[1]
        ultima = r[2]
        cursor.execute("SELECT nome FROM solicitacoes WHERE cpf = %s ORDER BY id DESC LIMIT 1", (cpf_cripto,))
        nome_row = cursor.fetchone()
        nome = nome_row[0] if nome_row else 'Não informado'
        recorrencia.append({'cpf': formatar_cpf(descriptografar_cpf(cpf_cripto)), 'nome': nome, 'total_recebido': total, 'ultima_entrega': str(ultima) if ultima else 'N/A'})
    lista_meses = []
    for i in range(12):
        ...
        lista_meses.append({'valor': valor, 'nome': f"{nome_mes_calc} de {data.strftime('%Y')}"})
    conexao.close()
    return render_template("relatorio.html", mes=mes, nome_mes=nome_mes, ano=ano, total_solicitacoes=total_solicitacoes, total_entregues=total_entregues, total_ausentes=total_ausentes, total_pendentes=total_pendentes, por_cras=por_cras, ultimas_entregas=ultimas_entregas, por_tecnico=por_tecnico, lista_meses=lista_meses, recorrencia=recorrencia, datetime=datetime, current_user=current_user)
...
def backup_automatico():
    ...
    try:
        ...
        for u in usuarios_raw:
            usuarios.append({'id': u[0], 'usuario': u[1], 'nome': u[2], 'perfil': u[3], 'cras': u[4], 'primeiro_acesso': u[5]})
        colunas = ['id', 'tecnico', 'cpf', 'cpf_hash', 'nome', 'data_nascimento', 'telefone', 'email', 'endereco', 'numero', 'complemento', 'bairro', 'cep', 'referencia', 'cras', 'data_escuta', 'total_pessoas', 'composicao_familiar', 'renda_bruta', 'renda_per_capita', 'beneficios', 'vulnerabilidade', 'servicos_suas', 'parecer', 'status', 'data_solicitacao', 'data_entrega', 'tecnico_entrega']
        solicitacoes = []
        ...
        backup = {'data': datetime.now(FUSO_RONDONIA).strftime('%d/%m/%Y %H:%M:%S'), 'total_usuarios': len(usuarios), 'total_solicitacoes': len(solicitacoes), 'usuarios': usuarios, 'solicitacoes': solicitacoes}
        ...
        if 'download' in request.path:
            return send_file(caminho_completo, as_attachment=True, download_name=nome_arquivo, mimetype='application/json')
